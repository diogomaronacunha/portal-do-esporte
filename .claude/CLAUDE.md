...
## Token Efficiency — Regras Obrigatórias

Estas regras são OBRIGATÓRIAS para controlar consumo de tokens da API Anthropic.

### Regra 1 — Agentes (CRÍTICO)
- **NUNCA reativar** um agente que já foi ativado na sessão atual
- Antes de chamar o Skill tool, verificar se o agente já está ativo
- Se já ativo: apenas continuar enviando comandos, sem recarregar
- Hook automático em `.claude/hooks/agent-session-guard.js` bloqueia duplicatas

### Regra 2 — Arquivos Grandes
- Arquivos > 100 KB: usar Grep ou Read com offset/limit
- Arquivos > 400 KB: NUNCA ler completo — usar Grep para localizar seções
- Workflows n8n (JSON): ler apenas os nós relevantes via Grep, não o arquivo completo
- Hook automático em `.claude/hooks/large-file-guard.js` bloqueia leituras excessivas

### Regra 3 — Sessões Longas
- Após 20+ trocas de mensagem: sugerir `/compact` para comprimir o contexto
- Ao mudar de projeto (social → app mobile → n8n): iniciar nova conversa
- Não reler arquivos já lidos nesta sessão — usar o conteúdo já disponível no contexto

### Regra 4 — Buscas e Ferramentas
- Preferir Grep sobre Read para localizar conteúdo
- Preferir Glob sobre Bash(find) para localizar arquivos
- Combinar múltiplas ferramentas independentes em paralelo (uma única mensagem)
- Não fazer WebSearch + WebFetch para o mesmo conteúdo — escolher um

### Regra 5 — Memória entre sessões
- Salvar decisões importantes em `.claude/projects/.../memory/MEMORY.md`
- Na próxima sessão: ler MEMORY.md em vez de reler todos os arquivos do projeto
- Não reler briefing.md, brand-identity.md, etc. se o conteúdo já está em MEMORY.md
...
| `@crab` | Krak | Análise de risco pré-implementação (9 dimensões) |
...
### Caranguejo Squad Protocol (OPT-IN)
- **@crab recomendado para:** epics novos, decisões de infraestrutura, integrações externas, mudanças de arquitetura
- **@crab opcional para:** ajustes de workflow n8n, conteúdo social media, correções pontuais, configurações
- `*review {input}` — análise completa em 9 dimensões (Fundação · Alicerce · Base/Dados · Estrutura · Visual · Segurança · Escala · Tempo · Ecossistema)
- `*quick-review {input}` — análise rápida nas 3 dimensões críticas
- **Findings CRÍTICOS bloqueiam todos os outros agentes** — nada avança sem resolução
- Findings ALTOS/MÉDIOS/BAIXOS documentados como risco aceito — não bloqueiam
- Objetivo: chegar melhor, não parar. Toda pinça abre uma porta melhor.

### When implementing features:
- Considerar @crab para decisões de alto impacto (ver Caranguejo Squad Protocol)
