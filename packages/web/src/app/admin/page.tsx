...
export default async function AdminDashboard() {
  ...
  const [noticias, eventos, atletas, clubes, lojistas, produtos, prestadores, ofertas] = await Promise.all([
    ...
    supabase.from('prestadores').select('id', { count: 'exact', head: true }).eq('status', 'pendente'),
  ...
  const total = (noticias.count ?? 0) + (eventos.count ?? 0) + (atletas.count ?? 0) + (clubes.count ?? 0) + (lojistas.count ?? 0) + (produtos.count ?? 0) + (prestadores.count ?? 0) + (ofertas.count ?? 0)
